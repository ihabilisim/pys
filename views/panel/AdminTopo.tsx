...
import { PolygonPoint, ChainageMarker, PolygonStatus, LandXMLFile } from '../../types';
import { ExcelUploadModal } from './modals/ExcelUploadModal';
import { MapWidget } from '../../components/MapWidget';
import { apiService } from '../../services/api';

export const AdminTopo: React.FC = () => {
    const { data, addPolygonPoint, updatePolygonPoint, addBulkPolygonPoints, deletePolygonPoint, addChainageMarker, updateChainageMarker, deleteChainageMarker, addLandXmlFile, deleteLandXmlFile } = useData();
    ...
    const handleSavePolygon = (e: React.FormEvent) => {
        e.preventDefault();
        ...
        if (editPolyId) { updatePolygonPoint(editPolyId, polyData); showToast('Poligon güncellendi.'); }
        else { addPolygonPoint(polyData); showToast('Poligon eklendi.'); }
        resetPolyForm();
    };
        ...
        if (e.target.files && e.target.files[0]) {
            ...
            if (error) {
                ...
                addLandXmlFile({
                    ...
                    isVisible: false // Default hidden to prevent lag
                });
                showToast('LandXML dosyası başarıyla eklendi.');
                // Reset form but keep last used color/opacity for convenience
    ...
    const activePolyCount = data.polygonPoints.filter(p => p.status === 'ACTIVE').length;
    ...
    return (
        <div className="space-y-6 animate-in fade-in pb-20">
            ...
            <div className="flex flex-col md:flex-row justify-between items-center gap-4 bg-iha-800 p-2 rounded-2xl border border-iha-700">
                ...
                {subTab === 'polygons' && (
                    <button onClick={() => setIsExcelModalOpen(true)} className="mr-2 px-6 py-2.5 bg-iha-700 hover:bg-iha-600 text-white rounded-xl text-xs font-bold flex items-center gap-2 border border-iha-600 transition-all">
                        <span className="material-symbols-outlined text-sm">table_view</span> Excel İçe Aktar
                    </button>
                )}
            </div>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 items-start">
                ...
                {subTab === 'landxml' ? (
                        ...
                        <div className="bg-iha-800 rounded-2xl border border-iha-700 shadow-xl overflow-hidden">
                            ...
                            <table className="w-full text-left text-sm text-slate-300">
                                ...
                                <tbody className="divide-y divide-iha-700">
                                    {data.landXmlFiles && data.landXmlFiles.length > 0 ? (
                                            ...
                                            <tr key={file.id} className="hover:bg-iha-900/50 transition-colors">
                                                ...
                                                <td className="p-4">
                                                    <div className="flex items-center gap-2 text-xs">
                                                        <div className="w-4 h-4 rounded border border-white/20" style={{backgroundColor: file.color, opacity: file.opacity}}></div>
                                                        <span className="font-mono">{file.opacity * 100}%</span>
                                                    </div>
                                                </td>
                                                <td className="p-4 text-xs text-slate-400">{file.description || '-'}</td>
                                                <td className="p-4 text-xs">
                                                    <div className="text-white">{file.uploadedBy}</div>
                                                    <div className="text-slate-500">{file.uploadDate}</div>
                                                </td>
                                                <td className="p-4 text-right">
                                                    <a href={file.url} target="_blank" rel="noopener noreferrer" className="inline-flex p-2 text-blue-400 hover:bg-blue-500/10 rounded mr-1"><span className="material-symbols-outlined text-lg">download</span></a>
                                                    <button onClick={() => {if(window.confirm('Dosyayı silmek istediğinize emin misiniz?')) deleteLandXmlFile(file.id)}} className="inline-flex p-2 text-red-400 hover:bg-red-500/10 rounded"><span className="material-symbols-outlined text-lg">delete</span></button>
                                                </td>
                                            </tr>
                                        ))
                                    ) : (
                                        <tr><td colSpan={6} className="p-8 text-center text-slate-500 italic">Henüz yüklenmiş LandXML dosyası yok.</td></tr>
                                    )}
                                </tbody>
                            </table>
                        </div>
                    ...
                    <>
                                ...
                                <div className="bg-iha-800 p-6 rounded-2xl border border-iha-700 shadow-xl animate-in slide-in-from-left-4">
                                    ...
                                    <form onSubmit={handleSavePolygon} className="space-y-3">
                                        ...
                                        <div>
                                            ...
                                            <select 
                                                value={newPoly.status} 
                                                onChange={e => setNewPoly({...newPoly, status: e.target.value as PolygonStatus})}
                                                className={`w-full border rounded-lg p-2.5 text-sm font-bold outline-none ${
                                                    newPoly.status === 'ACTIVE' ? 'bg-emerald-900/30 border-emerald-500/50 text-emerald-400' :
                                                    newPoly.status === 'LOST' ? 'bg-red-900/30 border-red-500/50 text-red-400' :
                                        ...
                                        <div className="grid grid-cols-2 gap-2">
                                            <input placeholder="P.No" value={newPoly.no} onChange={e => setNewPoly({...newPoly, no: e.target.value})} className="bg-iha-900 border border-iha-700 rounded-lg p-2.5 text-white text-sm" />
                                            <input placeholder="KM" value={newPoly.km} onChange={e => setNewPoly({...newPoly, km: e.target.value})} className="bg-iha-900 border border-iha-700 rounded-lg p-2.5 text-white text-sm" />
                                        </div>
                                        ...
                                        <div className="grid grid-cols-2 gap-2 pt-2 border-t border-iha-700">
                                            <input placeholder="Doğu (Y)" value={newPoly.east} onChange={e => setNewPoly({...newPoly, east: e.target.value})} className="bg-iha-900 border border-iha-700 rounded-lg p-2 text-white text-xs font-mono" />
                                            <input placeholder="Kuzey (X)" value={newPoly.north} onChange={e => setNewPoly({...newPoly, north: e.target.value})} className="bg-iha-900 border border-iha-700 rounded-lg p-2 text-white text-xs font-mono" />
                                        </div>
                        ...
                        {/* Liste Alanı (2/3) */}
                        <div className="lg:col-span-8 bg-iha-800 rounded-2xl border border-iha-700 shadow-2xl overflow-hidden">
                            <div className="p-4 bg-iha-900 border-b border-iha-700 flex justify-between items-center">
                                <span className="text-xs font-bold text-slate-400 uppercase tracking-widest flex items-center gap-2">
                                    {subTab === 'polygons' ? 'TOPLAM AKTİF POLİGON SAYISI:' : 'KM TAŞLARI LİSTESİ'}
                                    <span className="bg-slate-800 text-emerald-400 font-mono text-sm px-2 py-0.5 rounded border border-slate-700">
                                        {subTab === 'polygons' ? activePolyCount : data.chainageMarkers.length}
                                    </span>
                                </span>
                                {subTab === 'polygons' && (
                                    <input placeholder="Ara..." value={searchTerm} onChange={e => setSearchTerm(e.target.value)} className="bg-iha-800 border border-iha-700 rounded-lg px-3 py-1.5 text-xs text-white outline-none focus:border-emerald-500 w-48 transition-all focus:w-64" />
                                )}
                            </div>
                            {/* max-h-[280px] restricts to ~5 rows */}
                                    ...
                                    <thead className="bg-iha-900/50 text-[10px] uppercase font-bold text-slate-500 sticky top-0 z-10">
                                        {subTab === 'polygons' ? (
                                            <tr>
                                                ...
                                                <th className="p-4">Yapı / KM</th>
                                                <th className="p-4">Koordinatlar</th>
                                                <th className="p-4">Kot</th>
                                                <th className="p-4 text-right">İşlem</th>
                                            </tr>
                                        ) : (
                                            <tr><th className="p-4">Kilometre</th><th className="p-4">WGS84 Lat</th><th className="p-4">WGS84 Lng</th><th className="p-4 text-right">İşlem</th></tr>
                                        )}
                                    </thead>
                                    <tbody className="divide-y divide-iha-700">
                                        {subTab === 'polygons' ? (
                                            data.polygonPoints.filter(p => p.polygonNo.includes(searchTerm) || p.roadName?.includes(searchTerm)).map(p => (
                                                <tr key={p.id} className={`hover:bg-iha-900/50 group transition-colors ${p.status !== 'ACTIVE' ? 'opacity-60 bg-red-900/10' : ''}`}>
                                                    <td className="p-4">
                                                        <span className={`text-[9px] px-1.5 py-0.5 rounded font-bold uppercase border ${
                                                            p.status === 'ACTIVE' ? 'border-emerald-500/30 text-emerald-400 bg-emerald-500/10' : 
                                                            p.status === 'LOST' ? 'border-red-500/30 text-red-400 bg-red-500/10' : 
                                                            'border-orange-500/30 text-orange-400 bg-orange-500/10'
                                                        }`}>
                                                            {p.status === 'ACTIVE' ? 'Aktif' : p.status === 'LOST' ? 'Kayıp' : 'Hasarlı'}
                                                        </span>
                                                    </td>
                                                    ...
                                                    <td className="p-4 text-right">
                                                        <div className="flex justify-end gap-2 opacity-50 group-hover:opacity-100 transition-opacity">
                                                            <button onClick={() => { setEditPolyId(p.id); setNewPoly({no:p.polygonNo, roadName:p.roadName||'', km:p.km||'', offset:p.offset||'', east:p.east, north:p.north, elev:p.elevation, lat:p.lat||'', lng:p.lng||'', desc:p.description, status:p.status}); }} className="p-1.5 text-blue-400 hover:bg-blue-500/20 rounded"><span className="material-symbols-outlined text-sm">edit</span></button>
                                                            <button onClick={() => deletePolygonPoint(p.id)} className="p-1.5 text-red-400 hover:bg-red-500/20 rounded"><span className="material-symbols-outlined text-sm">delete</span></button>
                                                        </div>
                                                    </td>
                                                </tr>
                                            ))
                                            ...
                                            data.chainageMarkers.map(m => (
                                                <tr key={m.id} className="hover:bg-iha-900/50 group transition-colors">
                                                    <td className="p-4 font-bold text-white font-mono">{m.km}</td>
                                                    <td className="p-4 font-mono text-xs">{m.lat}</td>
                                                    <td className="p-4 font-mono text-xs">{m.lng}</td>
                                                    <td className="p-4 text-right">
                                                        <div className="flex justify-end gap-2 opacity-50 group-hover:opacity-100 transition-opacity">
                                                            <button onClick={() => { setEditKmId(m.id); setNewKm({km:m.km, lat:m.lat.toString(), lng:m.lng.toString(), align:m.align}); }} className="p-1.5 text-blue-400 hover:bg-blue-500/20 rounded"><span className="material-symbols-outlined text-sm">edit</span></button>
                                                            <button onClick={() => deleteChainageMarker(m.id)} className="p-1.5 text-red-400 hover:bg-red-500/20 rounded"><span className="material-symbols-outlined text-sm">delete</span></button>
                                                        </div>
                                                    </td>
                                                </tr>
                                            ))
            ...
            <ExcelUploadModal isOpen={isExcelModalOpen} onClose={() => setIsExcelModalOpen(false)} onUpload={addBulkPolygonPoints} />
        </div>
    );
};
