...
const SHORTCUT_MAP: Record<string, { label: string; tab: string; icon: string; color: string }> = {
    'manage_map': { label: 'Harita & Survey', tab: 'topo', icon: 'landscape', color: 'bg-emerald-500' },
    'manage_machinery': { label: 'Makine Parkı', tab: 'machinery', icon: 'agriculture', color: 'bg-orange-500' },
    'manage_materials': { label: 'Malzeme Yönetimi', tab: 'materials', icon: 'inventory_2', color: 'bg-purple-500' },
    'manage_daily_log': { label: 'Günlük Raporlar', tab: 'dashboard', icon: 'edit_note', color: 'bg-indigo-500' },
    'manage_files': { label: 'PVLA Dosyalar', tab: 'pvla', icon: 'folder_shared', color: 'bg-blue-500' },
    'manage_drone': { label: 'Drone Uçuşları', tab: 'drone', icon: 'flight', color: 'bg-red-500' },
    'manage_timeline': { label: 'İş Programı', tab: 'timeloc', icon: 'calendar_month', color: 'bg-teal-500' },
    'manage_quality': { label: 'Kalite Kontrol', tab: 'topo', icon: 'flag', color: 'bg-rose-500' }
};
    ...
    return (
        <div className="w-full md:w-1/2 p-6 bg-iha-800 flex flex-col">
            ...
            <p className="text-xs text-slate-400 mb-4 leading-relaxed">
                Sistem yetkilerinize tanımlanmış modüllere buradan hızlıca erişebilirsiniz.
            </p>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 overflow-y-auto custom-scrollbar flex-1 pr-1 content-start">
                {uniqueShortcuts.map((shortcut, idx) => (
                    ...
                    >
                        ...
                        <span className="text-xs font-bold text-slate-300 group-hover:text-white transition-colors">{shortcut.label}</span>
                    </button>
                ))}
                ...
                {uniqueShortcuts.length === 0 && (
                    <div className="col-span-full py-12 text-center border-2 border-dashed border-iha-700 rounded-xl flex flex-col items-center justify-center">
                        ...
                        <p className="text-xs text-slate-500">Özel yetki tanımlanmamış.</p>
                    </div>
                )}
            </div>
            ...
            <div className="mt-4 pt-4 border-t border-iha-700">
                <div className="flex items-center gap-3 p-3 bg-blue-900/20 rounded-xl border border-blue-500/20">
                    ...
                    <div className="text-xs text-blue-200">
                        <p className="font-bold">Yardım mı lazım?</p>
                        <p className="opacity-70">Yetki değişiklikleri için IT birimi ile görüşünüz.</p>
                    </div>
                </div>
            </div>
        </div>
